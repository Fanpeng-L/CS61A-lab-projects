    ...
    if len(reference_words) == 0 and len(typed_words) == 0:
        ...
        while i < len(typed_words) and i < len(reference_words):
            if typed_words[i] == reference_words[i]:
                correct += 1
            i += 1
        return round(correct / len(typed_words) * 100, 2)
...
def wpm(typed, elapsed):
    ...
    words per minute is not based on the number of words typed, but instead the 
    number of groups of 5 characters, so that a typing test is not biased by the
     length of words. The formula for words per minute is the ratio of the number 
     of characters (including spaces) typed divided by 5 (a typical word length) 
     to the elapsed time in minutes.
     For example, the string "I am glad!" contains three words and ten characters 
     (not including the quotation marks). The words per minute calculation uses 
     2 as the number of words typed (because 10 / 5 = 2). If someone typed this 
     string in 30 seconds (half a minute), their speed would be 4 words per minute.
...
def feline_fixes(typed, reference, limit):
    ...
    if len(typed) == 0 or len(reference) == 0:
        ...
        if typed[0] != reference[0]:
            return feline_fixes(typed[1:], reference[1:], limit - 1)
        else:
            return feline_fixes()
